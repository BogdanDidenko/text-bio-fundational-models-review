#!/usr/bin/env python3
"""Apply the input-role integrity adjudications and render the resulting crop set."""

from __future__ import annotations

import json
import shutil
from pathlib import Path
from typing import Any

from build_atlas_crop_crossvalidation_ledger import figure_manifests, png_dimensions
from prepare_atlas_crop_crossvalidation import (
    OUTPUT_ROOT as RUN_ROOT,
    ROOT,
    compact_route,
    make_crop_panel,
    read_json,
    write_json,
)


ATLAS_PATH = ROOT / "docs/input-representation-atlas/data/atlas.json"


def load_batch_reviews(role: str) -> dict[str, dict[str, Any]]:
    result = {}
    for path in (RUN_ROOT / "subagents" / role).glob("batch_*/response.json"):
        for review in read_json(path)["reviews"]:
            result[review["model_id"]] = review
    return result


def load_single(role: str, key: str) -> dict[str, dict[str, Any]]:
    result = {}
    for path in (RUN_ROOT / "subagents" / role).glob("*/response.json"):
        value = read_json(path)[key]
        result[value["model_id"]] = value
    return result


def main() -> int:
    atlas = read_json(ATLAS_PATH)
    architectures = {item["model_id"]: item for item in atlas["architectures"]}
    provisional = read_json(RUN_ROOT / "provisional_crossvalidated_crop_ledger.json")
    role_reviews = load_batch_reviews("input_role_integrity_validator")
    adjudications = load_single("input_role_adjudicator", "adjudication")
    replacements = load_single("input_role_replacement_cropper", "crop")
    figures = figure_manifests()

    ledger = []
    for original in provisional:
        model_id = original["model_id"]
        if original["status"] == "no_suitable_figure":
            ledger.append(original)
            continue
        review = role_reviews[model_id]
        cross_validation = dict(original.get("cross_validation") or {})
        cross_validation["input_role_integrity_validation"] = review
        if review["decision"] == "pass":
            item = dict(original)
            item["cross_validation"] = cross_validation
            item["annotation_pass"] = f"{item['annotation_pass']}__input_role_pass"
            ledger.append(item)
            continue

        decision = adjudications[model_id]
        cross_validation["input_role_adjudication"] = decision
        if decision["decision"] == "no_suitable_figure":
            ledger.append(
                {
                    "model_id": model_id,
                    "record_id": original["record_id"],
                    "status": "no_suitable_figure",
                    "figure": None,
                    "crop_box": None,
                    "panel_label": "",
                    "visible_input_object": "",
                    "visible_model_interface": "",
                    "figure_suitability": "unsuitable",
                    "confidence": decision["confidence"],
                    "rationale": decision["rationale"],
                    "annotation_pass": "input_role_integrity_reject_plus_scope_adjudicator_no_figure",
                    "cross_validation": cross_validation,
                }
            )
            continue
        if decision["decision"] in {"accept_current", "adjust_current"}:
            item = dict(original)
            if decision["final_crop_box"] is not None:
                item["crop_box"] = decision["final_crop_box"]
            item["confidence"] = decision["confidence"]
            item["rationale"] = decision["rationale"]
            item["annotation_pass"] = f"input_role_integrity_plus_scope_adjudicator_{decision['decision']}"
            item["cross_validation"] = cross_validation
            ledger.append(item)
            continue

        replacement = replacements[model_id]
        figure = figures[architectures[model_id]["record_id"]][int(replacement["figure_index"])]
        source = ROOT / figure["image_path"]
        width, height = png_dimensions(source)
        cross_validation["input_role_replacement_crop"] = replacement
        ledger.append(
            {
                "model_id": model_id,
                "record_id": original["record_id"],
                "status": "cropped_source_figure",
                "figure": {
                    "figure_index": int(figure["figure_index"]),
                    "image_path": figure["image_path"],
                    "caption": figure.get("caption") or "",
                    "page_no": figure.get("page_no"),
                    "pixel_width": width,
                    "pixel_height": height,
                },
                "crop_box": replacement["crop_box"],
                "panel_label": replacement["panel_label"],
                "visible_input_object": replacement["visible_input_object"],
                "visible_model_interface": replacement["visible_model_interface"],
                "figure_suitability": "suitable",
                "confidence": replacement["confidence"],
                "rationale": replacement["rationale"],
                "selection_rationale": decision["rationale"],
                "annotation_pass": "input_role_integrity_plus_scope_adjudicator_replacement_crop",
                "cross_validation": cross_validation,
            }
        )

    if len(ledger) != 111 or len({item["model_id"] for item in ledger}) != 111:
        raise RuntimeError("Post-role ledger must cover 111 unique models")
    crop_count = sum(item["status"] == "cropped_source_figure" for item in ledger)
    no_figure_count = sum(item["status"] == "no_suitable_figure" for item in ledger)
    if (crop_count, no_figure_count) != (81, 30):
        raise RuntimeError(f"Unexpected post-role counts: {crop_count}/{no_figure_count}")
    write_json(RUN_ROOT / "post_role_crossvalidated_crop_ledger.json", ledger)

    panel_root = RUN_ROOT / "review_panels/post_role_final_crops"
    if panel_root.exists():
        shutil.rmtree(panel_root)
    panel_root.mkdir(parents=True)
    manifest = []
    for item in ledger:
        if item["status"] != "cropped_source_figure":
            continue
        architecture = architectures[item["model_id"]]
        panel = make_crop_panel(item, panel_root / f"{item['model_id']}.jpg")
        manifest.append(
            {
                "model_id": item["model_id"],
                "model_name": architecture["model_name"],
                "paper_title": architecture["paper_title"],
                "record_id": architecture["record_id"],
                "final_figure_index": item["figure"]["figure_index"],
                "final_crop_box": item["crop_box"],
                "panel_path": panel["panel_path"],
                "panel_sha256": panel["panel_sha256"],
                "panel_layout": panel["panel_layout"],
                "routes": [compact_route(route) for route in architecture["routes"]],
            }
        )
    write_json(RUN_ROOT / "post_role_preview_manifest.json", manifest)
    report = {
        "status": "ok",
        "models": len(ledger),
        "crops": crop_count,
        "no_suitable_figure": no_figure_count,
        "role_gate_pass_without_adjudication": sum(
            item.get("annotation_pass", "").endswith("__input_role_pass") for item in ledger
        ),
        "role_adjudicator_accept_current": sum("scope_adjudicator_accept_current" in item.get("annotation_pass", "") for item in ledger),
        "role_adjudicator_adjust_current": sum("scope_adjudicator_adjust_current" in item.get("annotation_pass", "") for item in ledger),
        "role_adjudicator_replacement": sum("scope_adjudicator_replacement_crop" in item.get("annotation_pass", "") for item in ledger),
        "role_adjudicator_no_figure": sum("scope_adjudicator_no_figure" in item.get("annotation_pass", "") for item in ledger),
    }
    write_json(RUN_ROOT / "post_role_ledger_report.json", report)
    print(json.dumps(report, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
