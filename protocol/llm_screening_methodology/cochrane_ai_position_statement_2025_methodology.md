...
- Add an `AI use justification` subsection to [PRISMA_protocol.md](../PRISMA_protocol.md) that states why LLM assistance is being used for title/abstract screening, what risks it introduces, why those risks are acceptable in this review, and what safeguards are in place.
- Add a formal `human oversight policy` to [screening_process.md](../screening_process.md) defining the LLM's role, escalation triggers, manual adjudication rules, and whether any direct auto-exclusion is permitted.
- Expand the `data/screening_log.csv` schema described in [screening_process.md](../screening_process.md) beyond final phase decisions to include model/provider/version, prompt version or hash, run date, decision source (`ai`, `human`, `hybrid`), AI rationale, human final rationale, reviewer/adjudicator identity or role, and verification status.
...
- Treat [screening_benchmark_and_tiers.md](../screening_benchmark_and_tiers.md) as part of the formal justification package rather than only an internal calibration note. It already encodes the most important task-specific ambiguity classes; the governance paper implies that such calibration artifacts should be cited in deployment decisions.
- Update [screening_prompt.md](../screening_prompt.md) so that `UNCERTAIN` is explicitly framed as an oversight safeguard, not merely an output label.
