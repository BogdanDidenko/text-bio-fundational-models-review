...
- downstream use of embeddings, labels, prompts, explanations, or text-derived
  features generated by an existing external LLM;
- benchmark/resource papers that are not primary model papers;
- biological-token modeling that uses language metaphors but has no natural
  language text, text descriptions, captions, prompts, documents, or explicit
  text-bio alignment;
